...
@onready var templates_list = $VBoxContainer/TemplatesList
@onready var metadata_editor = $VBoxContainer/MetadataEditor
@onready var template_name_field = $VBoxContainer/MetadataEditor/HBoxContainer/TemplateName
@onready var metadata_list = $VBoxContainer/MetadataEditor/ScrollContainer/MetadataList
...
func _add_metadata_field(key: String, value: String, type_id: int = 0) -> void:
	...
	remove_button.connect("pressed", _remove_metadata_field.bind(hbox))
	hbox.add_child(remove_button)

	metadata_list.add_child(hbox)
	metadata_items.append({"hbox": hbox, "key": key_input, "value": value_input, "type": type_option})

func _remove_metadata_field(hbox: HBoxContainer) -> void:
	for i in range(metadata_items.size()):
		if metadata_items[i]["hbox"] == hbox:
...
func set_selected_node(node: Node) -> void:
	...
	if node:
		...
		if not found and not node_type.is_empty():
			...
			if not template_manager.templates.has(current_node_type):
